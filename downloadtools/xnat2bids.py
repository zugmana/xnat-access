                    ...
                    for iext in ['.json', '.nii.gz', '.bvec', '.bval', '.tsv']:
                        ...
                        if os.path.isfile(oldfile):
                            newfile = os.path.join(dirout, 
                                                    '{}'.format(sdan_id),
                                                    'ses-{}'.format(D[sdan_id][acquisition_date].loc[oldfnam,'ses']),
                                                    D[sdan_id][acquisition_date].loc[oldfnam,'datatype'],
    ...
    #     # Sort out the fieldmaps (add the IntendedFor field to the JSON)
        for sdan_id in D:
            for acquisition_date in D[sdan_id]:
                ...
                intendedfor = []
                juststored  = True
                D[sdan_id][acquisition_date] = D[sdan_id][acquisition_date].sort_values('seriesnum')
                indices = D[sdan_id][acquisition_date].index.tolist()
                
                for oldfnam in indices:
                    if D[sdan_id][acquisition_date].loc[oldfnam,'has_fmap'] != 'no':
                        if juststored:
                            intendedfor = []
                            juststored  = False
                    if D[sdan_id][acquisition_date].loc[oldfnam,'has_fmap'] == 'after':
                        #print(oldfnam)
                        intendedfor.append(os.path.join('ses-{}'.format(D[sdan_id][acquisition_date].loc[oldfnam,'ses']), 
                                                        D[sdan_id][acquisition_date].loc[oldfnam,'datatype'], 
                                                        '{}.nii.gz'.format(D[sdan_id][acquisition_date].loc[oldfnam,'newfnam'])))
                    if D[sdan_id][acquisition_date].loc[oldfnam,'datatype'] == 'fmap' and len(intendedfor) > 0:
                        Jpath = os.path.join(dirout, 
                                                '{}'.format(sdan_id),
                                                'ses-{}'.format(D[sdan_id][acquisition_date].loc[oldfnam,'ses']),
                                                D[sdan_id][acquisition_date].loc[oldfnam,'datatype'],
                                                '{}.json'.format(D[sdan_id][acquisition_date].loc[oldfnam,'newfnam']))
                        #print(Jpath)
                        #print(os.path.join(curdir, '{}.json'.format(oldfnam)))
                        intendedfor.sort()
                        J = readjson(Jpath)
                        if 'IntendedFor' in J:
                        ...
                        writejson(J, Jpath)
                        #print(J, os.path.join(curdir, '{}.json'.format(oldfnam)))
                        juststored = True
                ...
                for oldfnam in indices:
                    if D[sdan_id][acquisition_date].loc[oldfnam,'has_fmap'] != 'no':
                        if juststored:
                            intendedfor = []
                            juststored  = False
                    if D[sdan_id][acquisition_date].loc[oldfnam,'has_fmap'] == 'before':
                        intendedfor.append(os.path.join('ses-{}'.format(D[sdan_id][acquisition_date].loc[oldfnam,'ses']), 
                                                        D[sdan_id][acquisition_date].loc[oldfnam,'datatype'], 
                                                        '{}.nii.gz'.format(D[sdan_id][acquisition_date].loc[oldfnam,'newfnam'])))
                    if D[sdan_id][acquisition_date].loc[oldfnam,'datatype'] == 'fmap' and len(intendedfor) > 0:
